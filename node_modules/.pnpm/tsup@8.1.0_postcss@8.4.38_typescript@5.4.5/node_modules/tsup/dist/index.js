...
var _chunkU246LYQRjs = require('./chunk-U246LYQR.js');
...
function cleanDtsFiles(options) {
  if (options.clean) {
    _chunkGQ77QZBOjs.removeFiles.call(void 0, ["**/*.d.{ts,mts,cts}"], options.outDir);
  }
}
  ...
  await Promise.all(
    [...Array.isArray(configData) ? configData : [configData]].map(
      async (item) => {
        ...
        logger3.info("CLI", `tsup v${_chunkU246LYQRjs.version}`);
        if (config.path) {
